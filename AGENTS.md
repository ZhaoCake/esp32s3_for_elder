...
- **远程求值**: `uv run tools/eval.py "<expression>"`
- **串口监听**: `uv run tools/monitor.py [秒数]`（默认5秒，0=持续监听）
- **板子状态**: `uv run tools/status.py`
...
### 查看板子状态
1. 执行 `uv run tools/status.py`
2. 查看内存、WiFi、运行时间等信息，部分查询失败会显示 [N/A]

### 远程查询变量/表达式
1. 执行 `uv run tools/eval.py "current_color"` — 查询全局变量
2. 执行 `uv run tools/eval.py "gc.mem_free()"` — 查询内存
3. 可查询任意 MicroPython 表达式：GPIO 状态、WiFi 状态、传感器值等
4. 表达式通过 mpremote resume eval 执行，不会重置板子状态
5. 需要标准库模块时用 `__import__()`：`uv run tools/eval.py "__import__('sys').platform"`

### 监听板子输出
1. 执行 `uv run tools/monitor.py`（默认监听5秒）
2. 执行 `uv run tools/monitor.py 30`（监听30秒）
3. 执行 `uv run tools/monitor.py 0`（持续监听，Ctrl+C 停止）
4. 输出带时间戳 `[HH:MM:SS.mmm]`，适合调试和观察运行时行为
5. 此工具被动监听，不会中断板子上正在运行的程序
